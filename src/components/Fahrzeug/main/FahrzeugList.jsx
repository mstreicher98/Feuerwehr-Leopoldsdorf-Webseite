...
import { Center, Divider, Grid, GridCol, Text, Title } from "@mantine/core";
...
export default function FahrzeugList() {
  ...
  useEffect(() => {
    axios.get(`/api/fuhrpark/aktiv`).then((fahrzeugRes) => {
    ...
    axios.get(`/api/fuhrpark/historisch`).then((fahrzeugRes) => {
      if (fahrzeugRes.data.data.length > 0) {
        const list = fahrzeugRes.data.data.map((fahrzeug) => {
          return (
            <FahrzeugCard
    ...
    axios.get(`/api/fuhrpark/inaktiv`).then((fahrzeugRes) => {
      if (fahrzeugRes.data.data.length > 0) {
        const list = fahrzeugRes.data.data.map((fahrzeug) => {
          return (
            <FahrzeugCard
